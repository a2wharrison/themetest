...
testsite_basedir = "/home/wp03/public_html/testsites/"
testsite_baseurl = "wpthemetest.waynethursby.com/testsites/"
images_path = "/home/wp03/public_html/theme-images"
gtmetrix_api_username = "wayne.thursby@1srq.com"
gtmetrix_api_password = ""

# The Target WordPress instance to create posts on
wp_path = "/home/wp03/public_html/testwp"
...
def load_theme_data(filename='featured.json'):
...
def generate_sites(themedata):
    ...
    for theme in themedata:
        ...
        prefix_db = wp_instance_name.translate('-_.') + "_"
        ...
        os.system(wp_cli_base + "core config --dbhost=localhost --dbname=wp03_themetest --dbprefix=%s --dbuser=wp03_themetest --dbpass=MYSwaterloo1815" % prefix_db)
    ...
    for theme in themedata:
        ...
        if readonly:
            ...
            while (test_state != "completed") and (test_state != "error"):
                ...
                log.debug("HTTP Returned: " % r.status_code)
                log.debug(r.text)
...
def create_wp_post(post_content, post_category, post_excerpt, post_title):
    ...
    wpcli_base = "/usr/bin/wp --path=%s " % wp_path
...
def post_pages(acfdata):
    ...
    for wp_theme, theme in acfdata.iteritems():
        ...
        import_command = wpcli_base + r"""post create --post_content='[themetest_results_full]' --post_category=theme-performance-test --post_excerpt="%s" --porcelain --post_title='%s' """ % (
            value_asc,
        ...
        #    f.write("%s,%s,%s\n" %  (theme['theme_slug'], post_id, theme['theme_name']))
...
def post_rundown(acfdata):
    ...
    r = requests.get("http://wpthemetest.waynethursby.com/testwp/wp-json/wp/v2/posts", params=params)
    ...
    r = render('rundown-template.html', context=postdata)
    with open('rundown-output.html', "w") as f:
       f.write(r)
    post_id = create_wp_post('rundown-output.html', 3, "Hey everyone we're back here with another rundown.", "Rundown Today 4/19/2018" )
